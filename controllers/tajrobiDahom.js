...
export const create = async (req, res) => {
    ...
    try {      
        const user = await tajrobiDahom.find({studentId})
        if(user.length == 0){
            const newTopic = new tajrobiDahom({ ...tajrobiGet })
            await newTopic.save();
...
export const get = async (req, res) => {
    // res.send('GET WORKS!')
    try {
        const topicGet = await tajrobiDahom.find()
...
export const deleteAll = async (req, res) => {
    ...
    await tajrobiDahom.deleteMany({})
    
}

export const deleteById = async (req, res) => {
    const {id} = req.params;
    await tajrobiDahom.findByIdAndRemove(id)
 ...
 export const getBySearch = async (req, res) => {
    ...
    try {
        ...
        const topics = await tajrobiDahom.find({ Title })
...
export const getBySearchCode = async (req, res) => {
    ...
    try {
        ...
        const topics = await tajrobiDahom.find({ studentId })
        res.status(200).json(topics)
    } catch (error) {
        res.status(404).json({ message: error })
    }
...
export const updateById = async (req, res) => {
    ...
    try {
        ...
        if(topics.length>0){//exists
            ...
            await tajrobiDahom.updateOne({studentId:studentId},{$set:{
