...
import ensaniYazdahom from "../models/ensaniYazdahom.js";
...
export const create = async (req, res) => {
    ...
    try {      
        const user = await ensaniYazdahom.find({studentId})
        if(user.length == 0){
            const newTopic = new ensaniYazdahom({ ...ensaniGet })
            await newTopic.save();
...
export const get = async (req, res) => {
    // res.send('GET WORKS!')
    try {
        const topicGet = await ensaniYazdahom.find()
...
export const deleteAll = async (req, res) => {
    ...
    await ensaniYazdahom.deleteMany({})
    
}

export const deleteById = async (req, res) => {
    const {id} = req.params;
    await ensaniYazdahom.findByIdAndRemove(id)
