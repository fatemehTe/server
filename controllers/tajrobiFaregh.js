...
export const create = async (req, res) => {
    ...
    try {      
        const user = await tajrobiFaregh.find({studentId})
        if(user.length == 0){
            const newTopic = new tajrobiFaregh({ ...tajrobiGet })
            await newTopic.save();
...
export const get = async (req, res) => {
    // res.send('GET WORKS!')
    try {
        const topicGet = await tajrobiFaregh.find()
...
export const deleteAll = async (req, res) => {
    ...
    await tajrobiFaregh.deleteMany({})
    
}

export const deleteById = async (req, res) => {
    const {id} = req.params;
    await tajrobiFaregh.findByIdAndRemove(id)
 ...
 export const getBySearch = async (req, res) => {
    ...
    try {
        ...
        const topics = await tajrobiFaregh.find({ Title })
...
export const getBySearchCode = async (req, res) => {
    ...
    try {
        ...
        const topics = await tajrobiFaregh.find({ studentId })
        res.status(200).json(topics)
    } catch (error) {
        res.status(404).json({ message: error })
    }
...
export const updateById = async (req, res) => {
    ...
    try {
        ...
        if(topics.length>0){//exists
            ...
            await tajrobiFaregh.updateOne({studentId:studentId},{$set:{
...
//    return res.status(200).json(req.query)
    let code = '13'
    try {
        const topics = await tajrobiFaregh.find({ studentId })//studentId , topicsDetails
